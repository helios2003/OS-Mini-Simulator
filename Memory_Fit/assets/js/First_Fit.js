...
function print_num(){
    ...
    add_new.style.backgroundColor="lightblue";
...
function print_file(){
    ...
    show_Aloc_field.style.backgroundColor="lightblue";
...
function inner_cal() {
    ...
    for(let i=0;i<Avail_Mem.length;i++)
    {
        ...
        {
            finalprinter.innerHTML+=`<div class="progress" role="progressbar" aria-label="Basic example" aria-valuenow="${parseFloat((parseInt(Avail_Mem[i])/parseInt(Avail_before[i])))*100}" aria-valuemin="0" aria-valuemax="100" style="width:25%;margin:20px;background-color:red">
            <div class="progress-bar" style="width:${parseFloat((parseInt(Avail_Mem[i])/parseInt(Avail_before[i])))*100}%"></div>
            </div>`;
        }
